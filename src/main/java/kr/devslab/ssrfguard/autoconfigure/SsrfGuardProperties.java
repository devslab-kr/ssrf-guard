...
/**
 * Configuration for the SSRF Guard starter — exposed under the {@code ssrf.guard}
 ...
 *
 * <p>Defense in depth: the interceptor checks scheme/host/port up-front, the
 * DNS resolver re-checks the host AND filters private/loopback/link-local
 * IPs (under {@link #blockPrivateNetworks}), and — when
 * {@link #followRedirects} is true — every redirect hop is re-validated
 * through the same DNS resolver. This is what closes the TOCTOU window a
 * naive whitelist check would leave open.
 */
@Data
...
public class SsrfGuardProperties {
    ...
    /**
     * Master switch — when false no SSRF Guard beans are registered and Spring
     * Boot's stock {@code RestClient.Builder} is used as-is. Default: true.
     */
    ...
    /**
     * Reserved for future use. Today every request resolves DNS through
     * {@link kr.devslab.ssrfguard.security.SafeDnsResolver}; consumers who
     * want JVM-wide DNS caching should set {@code networkaddress.cache.ttl}
     * directly. Flipping this property currently has no effect.
     */

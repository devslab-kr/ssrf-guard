...
ssrf:
  guard:
    ...
    llm:
      scan-embedded: false          # 기본; true = 문장 중간 URL도 탐지
```

수동(자동 wrap 미사용) 사용 시에는 `JsonToolInputGuard`에 직접 플래그를 넘긴다:

```java
var guard = new JsonToolInputGuard(policy, /* throwOnViolation */ false, /* scanEmbedded */ true);
var safe = new SsrfGuardedToolCallback(rawCallback, guard);   // 또는 SsrfGuardedToolExecutor
```
